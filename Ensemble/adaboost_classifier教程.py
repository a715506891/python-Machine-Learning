...
def test_AdaBoostClassifier(*data):
    ...
    estimators_num = len(clf.estimators_)  # 一共多少个分类器
    X = range(1, estimators_num + 1)
    ax.plot(list(X), list(clf.staged_score(
        X_train, y_train)), label="Traing score")
    ax.plot(list(X), list(clf.staged_score(
        X_test, y_test)), label="Testing score")
...
def test_AdaBoostClassifier_base_classifier(*data):
    ...
    :return:  None
    '''
    from sklearn.naive_bayes import GaussianNB  # 基础分类器 的一种
    X_train, X_test, y_train, y_test = data
    fig = plt.figure()
    ...
    clf = ensemble.AdaBoostClassifier(
        learning_rate=0.1, base_estimator=GaussianNB())
    clf.fit(X_train, y_train)
    # 绘图
    estimators_num = len(clf.estimators_)
    X = range(1, estimators_num + 1)
...
if __name__ == '__main__':
    ...
    # test_AdaBoostClassifier_algorithm(X_train,X_test,y_train,y_test) # 调用
