...
def load_data():
    ...
    rng.shuffle(indices)  # 混洗样本下标集合shuffle，随机排序
    ...
    for alpha, color in zip(alphas, colors):
        ...
        for gamma in gammas:
            clf = LabelPropagation(
                max_iter=100, gamma=gamma, alpha=alpha, kernel='rbf')
            clf.fit(X, y_train)
            scores.append(
                clf.score(X[unlabeled_indices], y[unlabeled_indices]))
...
def test_LabelPropagation_knn(*data):
   ...
   测试 LabelPropagation 的 knn 核时，预测性能随 alpha 和 n_neighbors 的变化
    ...
    for alpha, color in zip(alphas, colors):
        ...
        for K in Ks:
            clf = LabelPropagation(
                max_iter=100, n_neighbors=K, alpha=alpha, kernel='knn')
            clf.fit(X, y_train)
            scores.append(
                clf.score(X[unlabeled_indices], y[unlabeled_indices]))
